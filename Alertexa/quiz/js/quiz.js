...
async function startFaceMonitoring(videoEl) {
    // Wait for Face API to load completely
    await faceapi.nets.tinyFaceDetector.loadFromUri("/models");

    let multipleFaceWarningCount = 0;
    const maxMultipleFaceWarnings = 1; // direct submit after one detection

    async function detectFaces() {
        const detections = await faceapi.detectAllFaces(videoEl, new faceapi.TinyFaceDetectorOptions());
        if (detections.length > 1) {
            multipleFaceWarningCount++;
            showOnScreenMessage(`Multiple faces detected! Submitting test...`, 3000);
